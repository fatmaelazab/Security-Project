        ...
        try:
            ...
            while self.status:
                ...
                while self.buff.count(';')>=4:
                    if("usr;;;" in self.buff):
                        if(not self.buff.find("usr;;;") == 0):
                            start = self.buff.find("usr;;;")
                ...
                if '!##!' in str(echodane):
                    l= str(echodane).split('!##!')
                    concat= concat + l[1]
                    k=1
                elif k==1:
                    concat= concat + echodane
                else:
                    self.buff+=echodane    

                if '##endoffile##' in echodane:
                    done=True
                    concat= concat.replace('##endoffile##','')
                    encoded_s = concat.encode()
                    fh = open("/Users/SalmaOssama/Desktop/rev.png", "wb")
                    fh.write(base64.b64decode(encoded_s))
                    fh.close()

                    clear_message = lsb.reveal("/Users/SalmaOssama/Desktop/rev.png")

                    self.buff= l[0] + clear_message
                    concat = ''
                    k=0 
...
while password is None:        
    password = input('password: ') # This is input in the form of a string
    password_bytes = password.encode() # Convert to type bytes
    ...
    key = base64.urlsafe_b64encode(kdf.derive(password_bytes)) # Can only use kdf once
    f = Fernet(key)
    encrypted = f.encrypt(password_bytes)

    msg='password;'+str(key)+';'+str(encrypted)[0:40]+';'+str(encrypted)[40:]+';'  
    s.send(bytes(msg, 'UTF-8'))
